...
@Slf4j
@Data
public class ThrowFireballsImpl extends IWeapon {
    final private String name = "投火球";
    final private String msg = "可以对四周的某个方位进行一次中等距离常规输出,具有范围伤害特效";
    final private int active = 4;//攻击行动消耗
    final private int atkMaxDistance = 14;//攻击最大距离
    final private int atkMinDistance = 4;//攻击最小距离
    ...
    public void calcHurt(RoleModel role, RoleModel enemyRole) {
        ...
        diffractionCalc(role, enemyRole, 1, 0.3);
        diffractionCalc(role, enemyRole, 2, 0.1);
    }
     ...
     * @param enemyRole   敌方角色
     * @param distance    距离
     * @param diffraction 衍射倍率
     */
    private void diffractionCalc(RoleModel role, RoleModel enemyRole, int distance, double diffraction) {
        List<RoleModel> enemyRoleList = enemyRole.calcDistanceTeammateRoleList(distance);
        for (RoleModel diffractionRole : enemyRoleList) {
            damageMultiplier(role, diffractionRole, diffraction);
        }
    }
